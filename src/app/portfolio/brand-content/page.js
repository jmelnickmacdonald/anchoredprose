  ...
  return (
    <div className="subpage">
      ...
      <div className="subpage-portfolio-grid">
        {PIECES.map((p, i) => (
          <div key={i} className="portfolio-card" style={{ cursor: p.video ? 'pointer' : 'default' }} onClick={() => p.video && setLightbox(p.video)}>
            ...
            {p.video && (
              <div style={{ margin: '1rem 0', borderRadius: '4px', overflow: 'hidden', position: 'relative', background: '#0D1B2E', cursor: 'pointer' }} onClick={() => setLightbox(p.video)}>
